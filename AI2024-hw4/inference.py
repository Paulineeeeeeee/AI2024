...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # Generate all possible positions within the game grid
    all_positions = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]

    # Assign the same domain of positions to Pacman and both ghosts
    for variable in [PAC, GHOST0, GHOST1]:
        variableDomainsDict[variable] = all_positions

    # Compute the range for observed distances considering the maximum noise
    observed_distance_range = list(range(X_RANGE + Y_RANGE + MAX_NOISE - 1))

    # Assign the domain of observed distances to both observations
    variableDomainsDict[OBS0] = observed_distance_range
    variableDomainsDict[OBS1] = observed_distance_range
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        if total_value == 0:
            return  # If the total is 0, do nothing to avoid division by zero.
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        if not self or sum(self.values()) == 0:
            return None  # If the distribution is empty or all values are 0, return None.
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition:
            # If the ghost is in jail, the noisy distance should always be None.
            return 1.0 if noisyDistance is None else 0.0

        if noisyDistance is None:
            # If noisy distance is None and the ghost is not in jail, this is not possible
            return 0.0

        # Compute the true Manhattan distance from Pacman to the ghost
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            # Get the probabilities of moving to a new position from the old position
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        particles_per_position = self.numParticles // len(positions)
        for position in positions:
            self.particles += [position] * particles_per_position

        # If particles are left due to rounding, randomly distribute the remaining
