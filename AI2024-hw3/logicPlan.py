...
def sentence1() -> Expr:
    ...
    B = Expr('B')
    C = Expr('C')
    return Expr('&', A | B, Expr('<=>', Expr('~', A), Expr('|', Expr('~', B), C)), Expr('|', Expr('~', A), Expr('~', B), C))
...
def sentence3() -> Expr:
    ...
    return conjoin([
        alive1 % ((alive0 & ~killed0) | (~alive0 & born0)),
        ~(alive0 & born0),
        born0
    ])
    util.raiseNotDefined()
...
def findModelUnderstandingCheck() -> Dict[Expr, bool]:
    ...
    """    
    a = FakeExpr('a')  # 創建一個 FakeExpr 實例，模擬 'a' 表達式
    return {a: True}
    "*** BEGIN YOUR CODE HERE ***"
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # 構建表示前提蘊含結論的反證：前提為真且結論為假的情形不應該有滿足的模型
    negated_conclusion = Expr('~', conclusion)
    test_expr = premise & negated_conclusion
    # 檢查這個聯合是否不可滿足
    model = findModel(test_expr)
    # 如果沒有滿足模型，則返回 True 表示蘊含關係成立，否則返回 False
...
def atLeastOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    """Constructs a CNF expression such that at least one of the literals is true."""
    """返回表示至少一個變量為真的表達式"""
    """返回表示至少一個變量為真的表達式，優化以避免不必要的操作"""
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    return PropSymbolExpr(pacman_str, x, y, time=now) % disjoin(possible_causes)  # Disjunction of all causes
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    # Generate the Pacman successor axiom for all possible starting points to (x0, y0) at time t = 0
    # Pacman is at (x0, y0) at time t = 0
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))
    # Pacman takes action0 at time t = 0
    KB.append(PropSymbolExpr(action0, time=0))
    # Pacman takes action1 at time t = 1
    KB.append(PropSymbolExpr(action1, time=1))
    KB.append(pacphysicsAxioms(0, all_coords, non_outer_wall_coords, walls_grid, None, allLegalSuccessorAxioms))

    # Add pacphysics axioms for t=1
    KB.append(pacphysicsAxioms(1, all_coords, non_outer_wall_coords, walls_grid, None, allLegalSuccessorAxioms))
    # Check if (x1, y1) is a wall
    # Find a model where Pacman is at (x1, y1) at time t = 1
    model_pacman_at_x1_y1 = findModel(conjoin(KB) & PropSymbolExpr(pacman_str, x1, y1, time=1))
    # Find a model where Pacman is not at (x1, y1) at time t = 1
    model_pacman_not_at_x1_y1 = findModel(conjoin(KB) & ~PropSymbolExpr(pacman_str, x1, y1, time=1))
...
def positionLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    KB.append(PropSymbolExpr(pacman_str, x0, y0,time=  0))
    # Attempt to find a path to the goal within 50 timesteps
    for t in range(50):
        # Pacman can only be at one location at each time step
        props = [PropSymbolExpr(pacman_str, x, y,time= t) for (x, y) in non_wall_coords]
        KB.append(exactlyOne(props))

        # Check for a satisfying model that includes Pacman being at the goal at time t
        model = findModel(conjoin(KB) & PropSymbolExpr(pacman_str, xg, yg, time=t))
        if model:
            return extractActionSequence(model, actions)
        
        # Pacman takes exactly one action per time step
        KB.append(exactlyOne([PropSymbolExpr(action, time=t) for action in actions]))
        
        # Add successor state axioms for the next time step
